...
| `set-status --workspace <dir> (--id <id> \| --company <name> --title <name>) --status <status>` | `npm run workspace:set-status -- --workspace <dir>` | Set a tracked role's application status (`interested`/`applied`/`interview`/`offer`/`rejected`/`withdrawn`/`ghosted`) and rebuild the tracker. `ghosted` is set manually only. | Implemented. |
...
| `gap-report --input <gaps.json> --workspace <dir> [--roleId <id>] [--roleTitle <title>]` | `npm run workspace:gap-report -- --workspace <dir>` | Validate agent-drafted gap-classification JSON against the four-type schema and render a markdown gap report. Advisory only. | Implemented. |
| `build-tracker --workspace <dir> [--format md\|html] [--output <file>] [--title <text>]` | `npm run workspace:tracker -- --workspace <dir>` | Render `outputs/tracker.md` (default) or an interactive `outputs/tracker.html` from `roles.tracked.json`, both with a per-role stale flag (see [`workspace-schemas.md`](workspace-schemas.md#staleness-computation)); the HTML format also gets a pipeline funnel section and sortable/filterable stale badges. The CLI also accepts `build` as an alias. | Implemented. |
