...
| 4. Tailor | The agent drafts a resume config for one job posting; `tailor` validates it, audits every claim against your evidence ledger, renders the DOCX, and tracks the role — all in one pass. Add `--cover-letter` to draft and render an evidence-audited cover letter alongside it. | [`docs/playbooks/tailor.md`](docs/playbooks/tailor.md) · `npm run workspace:tailor` |
| 5. Rendered DOCX | An evidence-backed resume (and, optionally, cover letter) lands at `outputs/resumes/<Company>/` (and `outputs/cover-letters/<Company>/`), landed **un-applied** so you review it first. | — |
...
| Keyword scoring | Score how much of an agent-extracted keyword list a resume config covers — inline via `tailor --keywords`, or standalone. Advisory only. | [`tailor.md`](docs/playbooks/tailor.md#step-31a-keyword-coverage-advisory---keywords) · `npm run workspace:score-keywords` | [Workspace schemas](docs/workspace-schemas.md#keyword-list-input-score-keywords) |
